...
def get_the_cheapest_big_mac_price_by_year(year):
    ...
    idx_min_item = min_df.loc[min_df_idx]
    min_answer = f"{idx_min_item['name']}({idx_min_item['iso_a3']}): ${round(idx_min_item['dollar_price'],2)}"
    return min_answer
...
def get_the_most_expensive_big_mac_price_by_year(year):
    ...
    idx_max_item = max_df.loc[max_df_idx]
    max_answer = f"{idx_max_item['name']}({idx_max_item['iso_a3']}): ${round(idx_max_item['dollar_price'],2)}"
    return max_answer
